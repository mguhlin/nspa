...
for(let i=0;i<data.length;i++){
 ...
 s.images.add({blob:new Uint8Array(await fs.readFile(root+'/2026/images/nspa-trust-transparency-ai/'+id+'.png')),contentType:'image/png',alt:data[i].title+' — '+data[i].subtitle+'. '+data[i].blocks.map(x=>x.join(': ')).join('; '),fit:'contain',position:{left:0,top:0,width:1280,height:720}});
